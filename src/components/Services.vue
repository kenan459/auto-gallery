<template>
  <div class="bg-[#3C3744]">
    <div class="mx-auto px-[50px] xl:px-[100px] 2xl:px-[150px] py-[60px] text-white">
      <div
        class="mx-auto font-bold text-center text-[50px] pb-[30px] text-white"
      >
        Our Services
      </div>

      <div class="grid grid-cols-3 py-10 gap-[50px] xl:gap-[50px]">
        <div class="col-span-1 text-center">
          <img
            class="w-[400px] 2xl:w-[400px] rounded-md mx-auto border-[1px] border-black"
            src="../assets/cars/asset-l1.jpg"
            alt=""
          />
          <div class="font-bold text-[34px] pt-4 text-[#F4E6B8]">
            Car Selection
          </div>
          <p class="px-[30px] xl:px-10px] 2xl:px-[70px] py-2 text-[20px] font-medium text-white">
            Choose your perfect ride from family cars to sports cars!
          </p>
        </div>
        <div class="col-span-1 text-center">
          <img
            class="w-[400px] 2xl:w-[400px] rounded-md mx-auto border-[1px] border-black"
            src="../assets/cars/asset-l2.jpg"
            alt=""
          />
          <div class="font-bold text-[34px] pt-4 text-[#F4E6B8]">Trade-Ins</div>
          <p class="px-[30px] xl:px-10px] 2xl:px-[70px] py-2 text-[20px] font-medium text-white">
            Swap your old ride for a shiny new set of wheels with ease!
          </p>
        </div>
        <div class="col-span-1 text-center">
          <img
            class="w-[400px] 2xl:w-[400px] rounded-md mx-auto border-[1px] border-black"
            src="../assets/cars/asset-l3.jpg"
            alt=""
          />
          <div class="font-bold text-[34px] pt-4 text-[#F4E6B8]">
            Financing Aid
          </div>
          <p class="px-[30px] xl:px-10px] 2xl:px-[70px] py-2 text-[20px] font-medium text-white">
            Get easy, flexible payment plans that fit your budget and drive away
            happy!
          </p>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
.card img {
  background-image: url("");
  background-position: center bottom;
  background-size: cover;
  overflow: hidden;
}
</style>
